...
- Offline trust scan on custom `--file` / remote `--url` sets (heuristics + denylist); warn by default for local files; fail closed under `--strict-trust` or remote `--url` (`trust_scan_failed`); never claims "safe"
