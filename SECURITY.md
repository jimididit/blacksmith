...
The gallery index is a curated list of pointers, not a security endorsement or signed package catalog. Gallery install and apply resolve an entry to its set URL and use the same remote trust pipeline and unsigned policy described above. The optional `--refresh` fetches the index from the project's raw GitHub URL and writes the validated result to the user configuration cache; it does not fetch or cache the referenced set YAML.
